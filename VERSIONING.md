...
Patch numbers advance by exactly one per release: `0.5.0` → `0.5.1` →
`0.5.2`. Never skip or jump patch numbers (`0.5.0` → `0.5.2` is wrong),
...
gemchain guard ask-web-search-mcp
gemchain update ask-web-search-mcp <new-version> --dry-run
gemchain update ask-web-search-mcp <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-web-search-mcp.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/web_search/mcp/version.rb` by gemchain, not by hand.
...
| `lib/ask/web_search/mcp/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.5.0` → `0.5.1`.
- Several bug fixes ship together: one release, `0.5.0` → `0.5.1` (never
  `0.5.2`).
- New backwards-compatible API: `0.5.0` → `0.6.0`.
- Breaking change while pre-1.0: `0.6.0` → `0.7.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.5.1` →
  `0.5.2`, via the gemchain cascade — never hand-published.
